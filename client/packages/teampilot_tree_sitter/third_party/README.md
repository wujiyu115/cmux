...
at any tag (their `src/` ships only `grammar.js` + `scanner.c`).
**tree-sitter-make** ships one, but generated by an old CLI (ABI 13) whose
parse-action macros MSVC rejects. Their vendored `parser.c`/`grammar.json`/
`node-types.json` were generated locally with `tree-sitter-cli 0.25.6` (ABI
14–15, within core v0.25.10's supported range) from the pinned commits above;
`tool/fetch_grammars.sh` reproduces this via its GENERATED_GRAMMARS pass.
